...
// Variables para la vista (valores por defecto vacios)
$errorLogin   = '';
$errorReg     = '';
$errorGoogle  = isset($_GET['error_google']) ? 'Hubo un problema al conectar con Google o se canceló el inicio de sesión.' : '';
$sourceLogin  = $_GET['source'] ?? ($_POST['source'] ?? '');
$modoActivo   = 'login';
$valores      = ['login_email' => '', 'nombre' => '', 'telefono' => '', 'email_registro' => ''];

// Mensaje contextual si el usuario llego desde el selector de reservas
$mensajeReserva = '';
if (isset($_SESSION['reserva_pendiente']) && is_array($_SESSION['reserva_pendiente'])) {
    $rp = $_SESSION['reserva_pendiente'];
    // Limpiar si es stale (más de 30 min desde que se guardó)
    if (($rp['guardada_en'] ?? 0) < time() - 1800) {
        unset($_SESSION['reserva_pendiente']);
    } else {
        $hora = isset($rp['hora']) ? ' a las ' . substr((string)$rp['hora'], 0, 5) : '';
        $mensajeReserva = 'Para confirmar tu cita de ' . ($rp['servicio_nombre'] ?? 'tu servicio')
                . ' el ' . ($rp['fecha_label'] ?? ($rp['fecha'] ?? '')) . $hora
                . ', accede a tu cuenta o crea una nueva. Solo tardas 30 segundos.';
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'registro') {
        ...
        // LOGIN: CSRF → campos vacios → formato email → credenciales
        if (!Csrf::validarToken('csrf_login', $_POST['csrf_token'] ?? null)) {
            $errorLogin = 'Sesión caducada. Recarga la página.';

        } else {
            $email    = trim($_POST['email'] ?? '');
            $password = (string)($_POST['password'] ?? '');
            $valores['login_email'] = $email; // repoblar el campo si hay error

            if ($email === '' || $password === '') {
                $errorLogin = 'Por favor, rellena todos los campos.';
...
// Token CSRF para los formularios de la vista
$csrfToken    = Csrf::generarToken('csrf_login');
$wrapperClase = 'login-wrapper ' . ($modoActivo === 'registro' ? 'es-registro' : '');
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <div class="flex h-full w-full max-sm:block max-sm:h-auto">
        ...
        <section class="flex h-full w-1/2 flex-col items-center justify-center bg-[#0a0a0a] px-12 py-14 max-sm:hidden max-sm:min-h-dvh max-sm:w-full max-sm:px-7 max-sm:pb-9 max-sm:pt-[72px] max-sm:group-[.es-registro]/auth:flex" aria-labelledby="registerTitle">
            ...
            <form method="post" action="login.php" class="w-full">
                ...
                <input class="mb-3 w-full rounded-md border border-[#282828] bg-[#141414] px-3.5 py-[13px] text-[13px] tracking-[0.03em] text-[#e0e0e0] outline-none transition placeholder:text-[#3a3a3a] focus:border-[var(--gold)] focus:bg-[#171717] focus:shadow-[0_0_0_2px_rgba(212,175,55,0.15)]" type="text" name="nombre" placeholder="Nombre completo" value="<?= h($valores['nombre']) ?>" autocomplete="name" required>
                <div class="relative mb-3 w-full">
                    <input class="mb-0 w-full rounded-md border border-[#282828] bg-[#141414] px-3.5 py-3 pr-10 text-xs tracking-[0.03em] text-[#e0e0e0] outline-none transition placeholder:text-[#3a3a3a] focus:border-[var(--gold)] focus:bg-[#171717] focus:shadow-[0_0_0_2px_rgba(212,175,55,0.08)]"
                           ...
                           value="<?= h($valores['telefono']) ?>" autocomplete="tel" required>
